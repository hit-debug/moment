...
| 상태 | 개발 준비 완료 (Vibe-Coding 입력용) |
| 핵심 기술 | React Native · Supabase · RevenueCat · AdMob · TanStack Query · Next.js · PostHog · Sentry |
| 업데이트 내역 | ① EAS Build·EAS Update·Sentry·PostHog 기술 스택 정식 반영 / ② WatchKit Phase 3 이동 / ③ 구독 요금 추후 결정 명시 / ④ 보관함 내보내기 .ics MVP 제외 / ⑤ 8장 Analytics 대시보드 신설 / ⑥ 게스트 좋아요 취소 서버 반영 (device_id 방식 B) / ⑦ 저널 최대 글자 수 한글 100자·영문 200자로 변경 |
...
| 스타일링 | Tailwind CSS + shadcn/ui (관리자) / NativeWind (RN 앱) |
...
| 텍스트 입력 | Placeholder: "이 글귀를 전하고 싶은 분에게 한 줄을 남겨보세요." 최대 **한글 100자 / 영문 200자** (글자 수 기준, 바이트 아님) |
| 저장 | journals(quote_id, user_id, text, created_at) Supabase 저장 |
...
| 미리보기 캔버스 | 명언 + 코멘트를 실시간 렌더링한 카드 미리보기 |
...
| 항목 | 내용 |
|------|------|
| 기본 테마 | 30종 자유 선택. 홈 화면 명언 캔버스에 즉시 반영 |
| 폰트 | 2종 자유 선택 |
...
**화면 구성**
- 저널을 최신순으로 노출.
- 각 저널 카드: 연결된 명언 원문 + 작성 날짜 + 저널 내용 미리보기.
- 카드 탭 시 R-010 상세 보기 진입.
- 삭제: 스와이프 또는 휴지통 아이콘.
...
### 4.17 R-015 — /main/my/settings/theme 테마 설정 (마이페이지)

R-021(modal/quote-customize)과 동일 컴포넌트. 기본 테마 30종·폰트 2종 선택, 시즌 테마팩. 위젯에도 동기화.
...
*Moment PRD v2.6 — End of Document*
